...
Divr
---------------------------------
Divr is a python package that uses pandas to divide a timeseries of depths into idividual dives.
...
def display_dive(index, df, starts):
    index = int(index)
    print str(starts.loc[index, 'start_block']) + ":" + str(starts.loc[index, 'end_block'])
    dive_profile = Dive(df[starts.loc[index, 'start_block']:starts.loc[index, 'end_block']])
...
def profile_dives(data, folder=None, columns={'depth': 'depth', 'time': 'time'}, acceleration_threshold=0.015, surface_threshold=3.0, ipython_display_mode=False):
    ...
    if ipython_display_mode:
        return interact(display_dive, index=widgets.IntSlider(min=0, max=starts.index.max(), step=1, value=0), df=fixed(data), starts=fixed(starts))
    elif folder is None:
